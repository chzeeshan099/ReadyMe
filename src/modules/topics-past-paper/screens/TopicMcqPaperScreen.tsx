...
export default function TopicMcqPaperScreen({ navigation, route }) {
  ...
  const mcqs = paperData?.item.mcqs ?? [];
  ...
  if (!paperData || !currentQuestion) {
    return (
      <LinearGradient colors={colors.bgGradient} className="flex-1">
        <SafeAreaView className="flex-1">
          <View className="flex-1 items-center justify-center px-6">
            <StatusBar backgroundColor={colors.header} barStyle={isDark ? "light-content" : "dark-content"} />
            <ViewerHeader
              title="MCQ Missing"
              subtitle="The requested topic MCQ paper is not available."
        ...
        <View className="flex-1">
          ...
          >
            <PdfCanvas
              ...
              topActions={[{ icon: "screen-rotation" }, { icon: "search" }, { icon: "download" }]}
            >
